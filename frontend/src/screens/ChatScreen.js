...
import {
  ...
  FaPlay, FaArrowDown, FaUserPlus, FaSignOutAlt, FaUser,
} from 'react-icons/fa';
import { useDispatch, useSelector } from 'react-redux';
import { setMessages, addMessage, updateMessageStatus, setSelectedChat, resetState, replaceMessage } from '../store';
import { saveMessages, clearOldMessages, deleteMessage, checkIndexes } from '../db';

console.log('Imported actions:', { setMessages, addMessage, updateMessageStatus, setSelectedChat, resetState, replaceMessage });
...
const ChatScreen = ({ token, userId, setAuth, socket }) => {
    ...
    try {
      ...
      const messages = await Promise.all(data.messages.map(async (msg) => {
        ...
        if (msg.senderId === userId) {
          newMsg.content = msg.plaintextContent || (msg.contentType === 'text' ? '[Missing Plaintext]' : '[Media Content]');
  ...
  useEffect(() => {
    ...
    const initializeChat = async () => {
      ...
      const keepAlive = setInterval(() => socket.emit('ping', { userId }), 10000);
    ...
    const setupSocketListeners = () => {
      ...
      socket.on('message', async (msg) => {
        ...
        const privateKeyPem = localStorage.getItem('privateKey');
        const existingMsg = chats[chatId]?.find((m) => m._id === msg._id || m.clientMessageId === msg.clientMessageId);

        if (existingMsg) return;

        let content = msg.senderId === userId ? msg.plaintextContent : msg.content;
        if (msg.recipientId === userId) {
          if (msg.contentType === 'text') {
            content = await decryptMessage(msg.content, privateKeyPem);
          } else if (['image', 'video', 'audio', 'document'].includes(msg.contentType)) {
            const decryptedContent = await decryptMessage(msg.content, privateKeyPem, true);
            content = URL.createObjectURL(new Blob([decryptedContent], { type: msg.contentType === 'document' ? 'application/pdf' : msg.contentType }));
          }
        }
        ...
        setUsers((prev) => {
          const userExists = prev.find((u) => u.id === chatId);
          const updatedLatestMessage = { ...newMsg, content };
            ...
            ? prev.map((u) =>
                u.id === chatId
                  ? {
                      ...
                      unreadCount: u.unreadCount + (msg.recipientId === userId && chatId !== selectedChat ? 1 : 0),
                    }
                  : u
              )
            : [
                ...
                {
                  id: chatId,
                  virtualNumber: msg.senderVirtualNumber || 'Unknown',
                  username: msg.senderUsername || '',
                  photo: msg.senderPhoto || 'https://placehold.co/40x40',
                  status: 'offline',
                  lastSeen: null,
                  latestMessage: updatedLatestMessage,
                  unreadCount: msg.recipientId === userId ? 1 : 0,
                  isSaved: false,
                },
              ];
          const sortedUsers = updatedUsers.sort((a, b) => new Date(b.latestMessage?.createdAt || 0) - new Date(a.latestMessage?.createdAt || 0));
          localStorage.setItem('cachedUsers', JSON.stringify(sortedUsers));
          return sortedUsers;
        });
      ...
      socket.on('messageStatus', ({ messageId, status }) => {
        Object.keys(chats).forEach((chatId) => {
          if (chats[chatId]?.some((msg) => msg._id === messageId)) {
      ...
      socket.on('newContact', (contactData) => {
        setUsers((prev) => {
          const userExists = prev.find((u) => u.id === contactData.id);
          if (!userExists) {
            const newUser = {
              ...contactData,
              isSaved: true,
            ...
            const updatedUsers = [...prev, newUser].sort((a, b) => new Date(b.latestMessage?.createdAt || 0) - new Date(a.latestMessage?.createdAt || 0));
            localStorage.setItem('cachedUsers', JSON.stringify(updatedUsers));
            return updatedUsers;
          }
          const updatedUsers = prev.map((u) => (u.id === contactData.id ? { ...u, ...contactData, isSaved: true } : u));
          localStorage.setItem('cachedUsers', JSON.stringify(updatedUsers));
          return updatedUsers;
        });
      });
  ...
  }, [token, userId, selectedChat, page, dispatch, fetchChatList, fetchMessages, chats, decryptMessage, hasMore, loading, socket]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const messageData = {
        ...
        plaintextContent: contentType === 'text' ? message : '',
      ...
      dispatch(replaceMessage({ recipientId: selectedChat, message: updatedMessage, replaceId: tempId }));
      await saveMessages([updatedMessage]);
    } catch (err) {
  ...
  const deleteMessages = async () => {
    try {
      const messageToDelete = (chats[selectedChat] || []).find((m) => m._id === showMessageMenu && m.senderId === userId);
      if (!messageToDelete) return;
      await axios.delete(`${BASE_URL}/social/message/${messageToDelete._id}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const forwardMessage = async (msg) => {
    ...
    try {
      ...
      await axios.post(`${BASE_URL}/social/message`, messageData, { headers: { Authorization: `Bearer ${token}` } });
    } catch (err) {
  ...
  const addContact = async () => {
    ...
    try {
      const { data } = await axios.post(`${BASE_URL}/auth/add_contact`, { userId, virtualNumber: newContactNumber }, { headers: { Authorization: `Bearer ${token}` } });
      setUsers((prev) => {
        const updatedUsers = prev.map((u) => (u.id === data.id ? { ...u, ...data, isSaved: true } : u));
        if (!updatedUsers.some((u) => u.id === data.id)) {
          updatedUsers.push({ ...data, isSaved: true, latestMessage: null, unreadCount: 0 });
        }
        const sortedUsers = updatedUsers.sort((a, b) => new Date(b.latestMessage?.createdAt || 0) - new Date(a.latestMessage?.createdAt || 0));
        localStorage.setItem('cachedUsers', JSON.stringify(sortedUsers));
        return sortedUsers;
  ...
  const saveUnsavedContact = async (contactId) => {
    try {
      const contact = users.find((u) => u.id === contactId);
      if (!contact || contact.isSaved) return;
      const { data } = await axios.post(`${BASE_URL}/auth/add_contact`, { userId, virtualNumber: contact.virtualNumber }, { headers: { Authorization: `Bearer ${token}` } });
      setUsers((prev) => {
        const updatedUsers = prev.map((u) => (u.id === contactId ? { ...u, ...data, isSaved: true } : u));
        localStorage.setItem('cachedUsers', JSON.stringify(updatedUsers));
        return updatedUsers;
      });
    } catch (err) {
        ...
        {selectedChat ? (
          <>
            <div className="bg-white dark:bg-gray-800 p-3 flex items-center justify-between border-b dark:border-gray-700 fixed top-0 md:left-[33.33%] md:w-2/3 left-0 right-0 z-10">
              ...
              {!users.find((u) => u.id === selectedChat)?.isSaved && (
                <FaUser onClick={() => saveUnsavedContact(selectedChat)} className="text-primary dark:text-gray-100 cursor-pointer" title="Save Contact" />
              )}
            </div>
                        ...
                        <div className="flex justify-between mt-1">
                          {msg.senderId === userId && (
                            <span className="text-xs">
                              {msg.status === 'sent' ? '✔' : msg.status === 'delivered' ? '✔✔' : <span className="text-blue-300">✔✔</span>}
                            </span>
                          )}
