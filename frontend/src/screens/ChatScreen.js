...
const ChatScreen = ({ token, userId, setAuth, socket, username, virtualNumber, photo, setSelectedChat }) => {
  ...
  const decryptMessage = async (encryptedContent, privateKeyPem) => {
    ...
    return encryptedContent; // Replace with actual decryption
  };
  ...
  const getPublicKey = async (userId) => {
    // Placeholder for fetching public key
    return 'mock-public-key'; // Replace with actual key fetching
  };
...
}, [selectedChat, fetchMessages, messages]);
...
useEffect(() => {
  ...
  socket.on('message', async (msg) => {
    ...
    if (msg.recipientId === userId) {
      ...
      const decryptedContent = msg.contentType === 'text' ? await decryptMessage(msg.content, privateKeyPem) : msg.content;
      setMessages((prev) => {
...
}, [socket, selectedChatId, userId, messages]);
...
const renderMessage = ({ index, indexstyle }) => {
  ...
  <div className="chat-screen">
    <div className="chat-header">
      ...
      <div className="chat-menu">
        ...
        <AnimatePresence showMenu={showMenu}>
          <motion.div
            ref={menuRef}
          ...
          >
            ...
            <div className="menu-item logout" onClick={handleLogout}>
              <FaSignOut className="fa-sign-out menu-item-icon" />
              Logout
            </div>
            {showAddContact && (
