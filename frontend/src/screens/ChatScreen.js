...
messageSchema.pre('save', async function (next) {
  try {
    await retryOperation(async () => {
      ...
      if (!sender) {
        sender = await User.findById(this.senderId).select('virtualNumber username photo status').lean();
        if (!sender) {
      ...
      if (this.isNew) {
        const existingMessage = await this.constructor.findOne({ clientMessageId: this.clientMessageId }).lean();
        if (existingMessage) {
          logger.warn('Duplicate message detected in pre-save', {
      ...
      if (this.isNew) {
        this.status = recipient.status === 'online' ? 'delivered' : 'sent';
    ...
    next(error); // Fail the save operation for critical errors
  }
});
...
const scheduleCleanup = () => {
  ...
  setInterval(async () => {
    try {
      await Message.cleanupOrphanedMessages();
    } catch (err) {
...
Message.cleanupOrphanedMessages().then(() => {
  scheduleCleanup();
}).catch(err => {
